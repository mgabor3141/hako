...
## Context
The broker holds the real upstream credentials (ADR-0002/0007). Since it's a
...
## Decision
Three tiers, all converging on the broker reading the secret from a tmpfs file
...
1. **Default — passphrase-vault (the seal/unseal model, à la Vault).** Secrets
   live **`age`-encrypted in a mounted folder**; the broker boots **sealed** and
   blocks until unsealed. `hako up` prompts for the passphrase (masked input)
   and pipes it to `docker exec -i broker hako-unlock` over **stdin** — never in
   args/env/history. The broker decrypts into **memory** and only then launches
   ...
   stored. The broker *is* the in-memory secret agent (ssh-agent/gpg-agent
...
processes** (process memory in the broker's namespace; default `ptrace_scope`),
...
**passphrase on every broker restart** (the deliberate seal/unseal tradeoff);
